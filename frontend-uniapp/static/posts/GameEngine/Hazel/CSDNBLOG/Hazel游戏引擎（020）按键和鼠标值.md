...
- 此节目的
  ...
  由019可以在程序任何地方检测**有一个**键是否按下、鼠标是否点击、鼠标位置等。

  但是如果要针对**某一个**按键(A)是否按下，需要使用GLFW宏定义的键值
  ...
  GLFW_KEY_A定义在#include <GLFW/glfw3.h>文件中，这样只要使用了GLFW_KEY_A就需要包含这个头文件，这样不好，应该自定义键值，从而**摆脱**与GLFW紧密联系在一起。
...
- 还为什么要自定义键值呢
  ...
  因为引擎未来想在mac和linux使用glfw，而在window上使用win32窗口，而win32窗口的键值与glfw窗口的键值**不一致**，所以需要自定义兼顾通用两个窗口库。

- 如何兼顾两个窗口库的方法分析（暂未实现）

  Cherno说了很多，我有点晕，大概就是

  - 根据**宏定义**

    比如：如果是win32窗口，自定义键值Tab=9，如果是glfw窗口，自定义键值Tab=258

  - 根据**函数映射**

    提供类似HZKEYToWin32Key(HZ_KEY key)的函数，根据输入HZ_KEY转换为Win32的键值
...
- SandboxApp
  ...
  ![](https://raw.githubusercontent.com/liujianjie/Image/main/ImgFloder/202306260024463.png)

  可见：输入轮询与事件系统都可以使用自定义的键值判断按键是否按下